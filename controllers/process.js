...
const lowvision = async (url) => {
  ...
  try {
    ...
    const filePath = path.resolve(__dirname, "..", "client", "build", fileName);
...
const securityHeader = async (url) => {
  return axios({
    method: "post",
    url: `${process.env.PYTHON_SERVER_URL}/securityheader`,
    data: {
      URL: url,
    },
  })
...
const sslapi = async (url) => {
  return axios({
    method: "post",
    url: `${process.env.PYTHON_SERVER_URL}/ssl`,
    data: {
      URL: url,
    },
  })
